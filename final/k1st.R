...
for(n in 5:150)
{
  ...
  for (i in 1:15)
  {
    ...
    for(j in 1:17)
    {
      s<- seq(0,5,0.3)
      tryCatch({
        tmpBeta <- optim(par=c(lm((response+0.1)^(1/alpha[i])~.,data=subset1)$coefficients),f,method='Nelder-Mead',x=X0,lambda=10^-s[i],alpha=alpha[i])
...
subset<-data1[,names(data1)%in%imp1]
subset1<-cbind(response=data1$response,subset)
g1<-lm(response~.,data=subset1)
plot(g1)
GMC(matrix(c(g1$fitted.values,as.numeric(subset1[,1])),ncol=2))[2]
sum(abs(g1$fitted.values-subset1[,1]))/566
plot(g1$residuals/sd(g1$residuals))

yhat<-ifelse(X0%*%tmpBeta$par>0,X0%*%tmpBeta$par+alpha[i],alpha[i]*(exp(X0%*%tmpBeta$par)-1)+alpha[i])
sum(abs(yhat-subset1[,1]))/566
yhat<-(X0%*%tmpBeta$par)^alpha[i]
sum(abs(yhat-subset1[,1]))/566
